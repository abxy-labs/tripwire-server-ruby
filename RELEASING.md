...
2. Create or configure the `foil-server` gem on RubyGems.
...
1. Bump `lib/foil/server/version.rb` to the target version in a pull request.
...
- reads the release version from `lib/foil/server/version.rb`
- reruns the equivalent of the repo checks
- builds the `.gem` artifact
- on real releases:
  - creates tag `vX.Y.Z`
  - publishes `foil-server` to RubyGems with trusted publishing
